...
stopword = list(['đến_nỗi', 'việc', 'các', 'bị', 'cho', 'đây', 'vẫn', 'đang', 'nếu', 'để', 'khi', 'lên', 'rằng', 'đó', 'vì', 'sự', 'lúc', 'của', 'sau', 'lại', 'và', 'so', 'cùng', 'nên', 'như', 'vào', 'cứ', 'gì', 'điều', 'cần', 'từng', 'vậy', 'với', 'chuyện', 'một_cách', 'chiếc', 'do', 'càng', 'cả', 'chỉ', 'bởi', 'cái', 'qua', 'ra', 'những', 'nữa', 'tại', 'sẽ', 'có_thể', 'nơi', 'là', 'rồi', 'từ', 'phải', 'theo', 'mà', 'chứ', 'thì', 'này', 'cũng'])
...
# Xử lí toàn bộ data
def preprocess_data(data_full_label):
    for i in range(0, len(data_full_label)):
        data_full_label.loc[i, "comment"] = text_preprocess(str(data_full_label.loc[i, "comment"]))
        print(i)
    data_full_label = data_full_label.dropna()
    return data_full_label
### Khi chạy thì gọi preprocess_data và truyền data frame vào
### Nhớ đổi lại cột "Comment" thành "comment" trong file excel
